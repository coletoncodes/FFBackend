...
struct PlaidItemDetailsRequest: Codable {
    ...
    init(
        ...
        self.options = PlaidItemOptions(accountIDs: plaidInstitutionAccountIds, currentDateString: currentDateISO8601)
    }
}
...
struct PlaidItemOptions: Codable {
    ...
    ///
    /// Format: date-time
    let min_last_updated_datetime: String?
    
    init(
        accountIDs: [String],
        currentDateString: String
    ) {
        self.account_ids = accountIDs
        self.min_last_updated_datetime = currentDateString
    }
}
